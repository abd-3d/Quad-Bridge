...
def bridge_odd_gap_inner(bm, top, bot, n_top, n_bot):
    ...
    for i in range(n_top - 1, top_center_right - 1, -1):
        ...
        if right_bot_idx < n_bot - 1 and right_bot_idx >= bot_center_right:
            bm.faces.new(
                (top[i], bot[right_bot_idx], bot[right_bot_idx + 1], top[i + 1])
            )
        else:
...
def bridge_odd_one_to_n(bm, top, bot, n_bot):
    ...
    for i in range(nr):
        ...
        rows.append(
            (
                bm.verts.new(ta_l.lerp(bot[bi_l].co, t)),
                bm.verts.new(ta_r.lerp(bot[bi_r].co, t)),
            )
        )
    bm.verts.ensure_lookup_table()
    ...
    for i in range(len(rows)):
        if i == 0:
            ...
            bm.faces.new(
                (rows[i][1], bot[n_bot - i - 1], bot[n_bot - i], rows[i - 1][1])
            )
...
def bridge_odd_two_to_n(bm, top, bot, n_bot):
    def get_top_u(u):
        return (
            top[0].co.lerp(top[1].co, u / 0.5)
            if u <= 0.5
            else top[1].co.lerp(top[2].co, (u - 0.5) / 0.5)
        )
...
def bridge_general_n_m(bm, top, bot, n_top, n_bot, flow_1to2_method):
    if n_top % 2 == 0 and n_bot % 2 == 1:
        ...
        def get_ct_u(u):
            return (
                c_top[0].co.lerp(c_top[1].co, u / 0.5)
                if u <= 0.5
                else c_top[1].co.lerp(c_top[2].co, (u - 0.5) / 0.5)
            )
