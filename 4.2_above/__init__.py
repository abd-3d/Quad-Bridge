...
def bridge_even_gap_two_inner(bm, top, bot, n_top, n_bot):
    side = (n_top - 2) // 2
    for i in range(side): bm.faces.new((top[i], top[i+1], bot[i+1], bot[i]))
    ts, bs = n_top - side, n_bot - side
    for i in range(side): bm.faces.new((top[ts+i], top[ts+i+1], bot[bs+i+1], bot[bs+i]))
...
def bridge_odd_one_to_n(bm, top, bot, n_bot):
    ...
    for i in range(len(rows) - 1): bm.faces.new((rows[i][0], rows[i+1][0], rows[i+1][1], rows[i][1]))
...
def bridge_even_two_to_n(bm, top, bot, n_bot):
    ...
    for i in range(nr): bm.faces.new((l_chain[i], bot[i], bot[i+1], l_chain[i+1]))
    r_chain = [top[2]] + [r[2] for r in rows]
    for i in range(nr): bm.faces.new((r_chain[i], r_chain[i+1], bot[n_bot-1-i], bot[n_bot-i]))
...
def bridge_general_n_m(bm, top, bot, n_top, n_bot, flow_1to2_method):
    if n_top % 2 == 0 and n_bot % 2 == 1:
        ...
        for i in range(side_len):
            if prev_mid: m_curr = prev_mid
            else: m_curr = bm.verts.new((top[i].co + bot[i].co)/2)
            ...
            prev_mid = m_next
            if i == side_len - 1: mid_l_anchor = m_next
        ...
        c_top = top[side_len : ts + 1]
        c_bot = bot[side_len : bs + 1]
        ...
        if not mid_l_anchor: mid_l_anchor = bm.verts.new((c_top[0].co + c_bot[0].co)/2)
        if not mid_r_anchor: mid_r_anchor = bm.verts.new((c_top[2].co + c_bot[-1].co)/2)
        ...
        if rem_hole_edges == 1: bm.faces.new((tri_l, tri_r, c_bot[2], c_bot[1]))
        else: bridge_odd_one_to_n(bm, [tri_l, tri_r], c_bot[1:-1], rem_hole_edges)
        return

    is_top_odd = (n_top % 2 == 1)
    side_len = (n_top - 1) // 2 if is_top_odd else (n_top - 2) // 2
    
    for i in range(side_len): bm.faces.new((top[i], top[i+1], bot[i+1], bot[i]))
    ts, bs = n_top - side_len, n_bot - side_len
    for i in range(side_len): bm.faces.new((top[ts+i], top[ts+i+1], bot[bs+i+1], bot[bs+i]))
    
    c_top, c_bot = top[side_len : ts + 1], bot[side_len : bs + 1]
    cn_bot = len(c_bot) - 1
    
    center_method = flow_1to2_method
    if len(c_top)-1 == 1 and cn_bot == 2: center_method = 0
    
    if is_top_odd:
        if cn_bot == 2: bridge_one_to_two_logic(bm, c_top, c_bot, center_method)
        elif cn_bot % 2 == 0: bridge_even_one_to_n(bm, c_top, c_bot, cn_bot)
        else: bridge_odd_one_to_n(bm, c_top, c_bot, cn_bot)
    else:
...
class MESH_OT_QuadBridge(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if not context.edit_object: return {'CANCELLED'}
        ...
        if t_type: self.topo_type = t_type
    ...
    def execute(self, context):
        ...
        elif n_top == 1 and n_bot % 2 == 1: bridge_odd_one_to_n(bm, top, bot, n_bot)
        elif n_top == 1 and n_bot % 2 == 0: bridge_even_one_to_n(bm, top, bot, n_bot)
        elif n_top == 2 and n_bot % 2 == 0: bridge_even_two_to_n(bm, top, bot, n_bot)
        elif n_top == 2 and n_bot % 2 == 1: bridge_odd_two_to_n(bm, top, bot, n_bot)
        else:
